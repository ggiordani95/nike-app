  ...
  return (
    <View style={{flex: 1, alignItems:'center', backgroundColor:'#f3f3f3', position:'relative'}}>
      ...
      <FlatList data={atFavorites} renderItem={({item,index}:any)=>(
        <Link href="/" style={{flexDirection:'row',height:height / 8, width: width * 0.9, justifyContent:'flex-start',alignItems:'center', backgroundColor:'#f3f3f3',marginVertical:12}} key={index}>
          <Image source={{ uri: item.image }} style={{height:height / 8, width:width * 0.2, borderWidth:1, borderColor:'#e7e7e7', borderRadius: 14}}/>
