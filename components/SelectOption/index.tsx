...
const SelectOption = ({ ...props }: ISelectOption) => {
  ...
  const reanimatedStyle = useAnimatedStyle<AnimatedStyleProp<any>>(() => {
    return {
  ...
  return (
    ...
    >
      <RowSpace style={{ width: "100%" }}>
        <SelectionView>
          <ViewHeaderSection style={{ fontSize: RFValue(12) }}>
            {props.headerText}
          </ViewHeaderSection>
          <CircleGroup>
              ...
              return (
                <Animated.View
                  style={[
                    ...
                    size == selectedSneaker && reanimatedStyle,
                  ]}
                  key={key}
                >
                  <Circle
                    onPress={() => selecting(size)}
                    style={[
                      {
                        width: CIRCLE_DIMENSION,
                        height: CIRCLE_DIMENSION,
                        borderRadius: height * 0.02,
                        backgroundColor:
                          selectedSneaker === size ? "#181818" : "#d3d3d3",
                      },
                    ]}
                    key={key}
                  >
